#!/usr/bin/env python3
"""
2D Flow Matching Ergodic Coverage Testbench with B-Splines (Alternative to CNF)

Refactors the discrete Euler LQR flow matching into a Continuous Normalizing Flow 
with a finite-dimensional B-Spline parameterization of the control inputs.
...
N_ITERS = int(os.environ.get("N_ITERS", 50000))   # flow matching iterations
step_size = 0.05  # flow matching step size
...
B_mat = jnp.array(basis_generator.B) # Shape: (T, NUM_CONTROL_POINTS)
...
def solve_v(C, s0, h_traj):
    """
    Solve the dense LQR equivalent system for the B-Spline control update V.
    """
...
def compile_jax_functions():
    ...
    def flow_matching_scan_step_inner(carry, _):
        ...
        # 3. Solve LQR for optimal B-spline velocity flow V
...
def init_bspline_from_positions(pos_trajs, dt):
    """
    Convert (N, T, 2) positions to (N, 2, K) B-Spline control points.
    """
...
def run_benchmark(out_dir: str, save_npy: bool = False, use_obstacle: bool = False):
    ...
    print(f"2D Flow Matching (B-Spline) | {N_PARTICLES} particles, T={T}, K={NUM_CONTROL_POINTS}, {N_ITERS} iters")
    ...
    plt.tight_layout()
    out = os.path.join(out_dir, f'flow_matching_bspline_2d_{os.environ.get("TARGET_SHAPE", "N")}_comparison.png')
...
if __name__ == "__main__":
    TARGET_SHAPE = os.environ.get('TARGET_SHAPE', 'N')
    run_benchmark(out_dir=f'/home/philipp/Documents/Uni/Master_thesis/results/Stein_Flow_matching_BSpline_{TARGET_SHAPE}')
