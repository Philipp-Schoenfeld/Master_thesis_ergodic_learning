#!/usr/bin/env python3
"""
2D Flow Matching Ergodic Coverage Testbench with Chebyshev Polynomials (Alternative to CNF)

Refactors the discrete Euler LQR flow matching into a Continuous Normalizing Flow 
with a finite-dimensional Chebyshev parameterization of the control inputs.
...
# ============================================================================
# 6. Chebyshev Flow Matching Formulation
# ============================================================================
...
def compute_G_and_d(C, s0, h_traj):
    ...
    M_flat = M.reshape(T, 4, 2 * NUM_COEFFS)
...
def solve_v(C, s0, h_traj):
    """
    Solve the dense LQR equivalent system for the Chebyshev control update V.
    """
    G, d = compute_G_and_d(C, s0, h_traj)
    # Solve G * vec(V) = d
    V_flat = jnp.linalg.solve(G, d)
    # Reshape back to (2, K)
    V = V_flat.reshape(2, NUM_COEFFS)
    return V
...
def compile_jax_functions():
    ...
    def flow_matching_scan_step_inner(carry, _):
        ...
        # 3. Solve LQR for optimal Chebyshev velocity flow V
...
# ============================================================================
# 7. Finite Differences to Initialise Chebyshev Control Points
# ============================================================================

def init_chebyshev_from_positions(pos_trajs, dt):
    """
    Convert (N, T, 2) positions to (N, 2, K) Chebyshev control points.
    """
...
def run_benchmark(out_dir: str, save_npy: bool = False, use_obstacle: bool = False):
    ...
    print(f"2D Flow Matching (Chebyshev) | {N_PARTICLES} particles, T={T}, K={NUM_COEFFS}, {N_ITERS} iters")
    ...
    for strat in strategies:
        ...
        C_init, x0_init = init_chebyshev_from_positions(pos_trajs, dt)
    ...
    plt.tight_layout()
    out = os.path.join(out_dir, f'flow_matching_chebyshev_2d_{os.environ.get("TARGET_SHAPE", "N")}_comparison.png')
...
if __name__ == "__main__":
    TARGET_SHAPE = os.environ.get('TARGET_SHAPE', 'N')
    run_benchmark(out_dir=f'/home/philipp/Documents/Uni/Master_thesis/results/Stein_Flow_matching_Chebyshev_{TARGET_SHAPE}')
