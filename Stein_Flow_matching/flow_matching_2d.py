#!/usr/bin/env python3
"""
2D Flow Matching Ergodic Coverage Testbench — Letter "N" target

Optimizes control inputs (u_traj) via LQR and Stein variational gradient flow 
so that simulated trajectories cover a target distribution shaped like the letter "N".

Uses five distinct trajectory initialization strategies from SE3_SVGD to compare performance.
"""
...
# ============================================================================
# 3. Hyperparameters (matching tsvec_2d.py)
# ============================================================================

T = 100           # trajectory steps
N_PARTICLES = 10  # number of trajectory particles
N_ITERS = 10000   # flow matching iterations (run inside Compiled scan)
...
# ============================================================================
# 5. JAX-differentiable Target Distribution
# ============================================================================

if TARGET_SHAPE == 'N':
    N_SEGMENTS_JNP = [
        (jnp.array([0.25, 0.15]), jnp.array([0.25, 0.85])),
        (jnp.array([0.25, 0.85]), jnp.array([0.75, 0.15])),
        (jnp.array([0.75, 0.15]), jnp.array([0.75, 0.85])),
    ]
elif TARGET_SHAPE == 'H':
    N_SEGMENTS_JNP = [
        (jnp.array([0.25, 0.15]), jnp.array([0.25, 0.85])),
        (jnp.array([0.75, 0.15]), jnp.array([0.75, 0.85])),
        (jnp.array([0.25, 0.50]), jnp.array([0.75, 0.50])),
    ]
elif TARGET_SHAPE == 'II':
    N_SEGMENTS_JNP = [
        (jnp.array([0.25, 0.15]), jnp.array([0.25, 0.85])),
...
def pdf(x):
    p = x[:2]
    dists = []
    for a, b in N_SEGMENTS_JNP:
        dists.append(jax_dist_to_segment(p, a, b))
...
# ============================================================================
# 6. Stein Variational Gradient in JAX
# ============================================================================
...
# ============================================================================
# 7. LQR Dynamics Model and Scan Loop Compilation
# ============================================================================
...
pointmass_lqr = PointMassLQR(dt=dt, x_dim=4, u_dim=2, Q=Q_mat, R=R_mat)
...
def compile_jax_functions():
    ...
    def flow_matching_scan_step_inner(carry, _):
        u_traj_all, x0_all = carry
        
        # 1. Simulate state trajectories
        x_trajs = vmap(pointmass_lqr.traj_sim, in_axes=(0, 0))(x0_all, u_traj_all)
        
        # 2. Reshape all state trajectories to compute collective Stein gradient
        all_x = x_trajs.reshape(-1, 4)
        stein_dx_all = stein_grad_jit(all_x, h=0.01)
        stein_dx_trajs = stein_dx_all.reshape(N_PARTICLES, T, 4)
        
        # 3. Solve LQR for all trajectory particles in parallel
        _, A_trajs, B_trajs = vmap(pointmass_lqr.linearize_dyn, in_axes=(0, 0))(x0_all, u_traj_all)
        v_trajs, _ = vmap(pointmass_lqr.solve, in_axes=(None, 0, 0, 0))(z0, A_trajs, B_trajs, stein_dx_trajs)
        
        # 4. Update control inputs
        u_traj_all_new = u_traj_all + step_size * v_trajs
        return (u_traj_all_new, x0_all), None
        
    flow_matching_scan_step = flow_matching_scan_step_inner

# ============================================================================
# 8. Finite Differences to Initialise LQR controls from Position Paths
# ============================================================================

def init_lqr_from_positions(pos_trajs, dt):
    # pos_trajs shape: (N_PARTICLES, T, 2)
    N, T, _ = pos_trajs.shape
...
def run_benchmark(out_dir: str, save_npy: bool = False, use_obstacle: bool = False):
    ...
    print(f"2D Flow Matching  |  {N_PARTICLES} particles, T={T}, K={K}, {N_ITERS} scan-iters")
    ...
    for strat in strategies:
        ...
        # 2. Extract matching control trajectory & initial states for LQR double integrator
        u_init, x0_init = init_lqr_from_positions(pos_trajs, dt)
        
        # Convert to JAX arrays
        u_traj_all = jnp.array(u_init)
        x0_all = jnp.array(x0_init)
        
        # 3. Simulate initial physical trajectories
        initial_x_trajs = vmap(pointmass_lqr.traj_sim, in_axes=(0, 0))(x0_all, u_traj_all)
        initial_pos = np.array(initial_x_trajs[:, :, :2]).reshape(N_PARTICLES, -1)
        
        # 4. JIT compilation & scan optimization loop
        (u_traj_all_opt, _), _ = lax.scan(flow_matching_scan_step, (u_traj_all, x0_all), None, length=N_ITERS)
        
        # 5. Simulate final physical trajectories
        final_x_trajs = vmap(pointmass_lqr.traj_sim, in_axes=(0, 0))(x0_all, u_traj_all_opt)
        final_pos = np.array(final_x_trajs[:, :, :2]).reshape(N_PARTICLES, -1)
    ...
    # ============================================================================
    # 10. Visualization (matching layout of tsvec_2d.py)
    # ============================================================================
    ...
    plt.tight_layout()
    out = os.path.join(out_dir, f'flow_matching_2d_{os.environ.get("TARGET_SHAPE", "N")}_comparison.png')
...
if __name__ == "__main__":
    TARGET_SHAPE = os.environ.get('TARGET_SHAPE', 'N')
    run_benchmark(out_dir=f'/home/philipp/Documents/Uni/Master_thesis/results/Stein_Flow_matching_{TARGET_SHAPE}')
