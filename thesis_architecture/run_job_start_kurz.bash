...
# Kurzer Lauf: Startpunkt-Konditionierung, CFM + ergodischer Zusatzterm.
# 200 Epochen, damit der Lauf im Zeitfenster wirklich *durchlaeuft* statt
# abgeschnitten zu werden — Ergebnisse am Dienstagvormittag.
#
...
ARGS="--D 384 --n_particles 256 --copies_per_char 75 --p_flip 0.0 \
      ...
      --tag K200 --keep_checkpoints 1 --db ergodic_dataset_start.db --use_wandb"

LATEST=$(ls -t checkpoints/*_START_FLAT*_K200_*_ep*.pt 2>/dev/null | head -1)
if [ -f "$LATEST" ]; then
    echo "Setze fort von: $LATEST"
    srun --unbuffered python flow_matching_runner_start.py --resume "$LATEST" $ARGS
else
    echo "Starte kurzen Lauf (Startpunkt-Konditionierung, CFM+ErgLoss, 200 Epochen)"
