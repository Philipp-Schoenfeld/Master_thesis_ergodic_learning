...
Datasets : all_shapes
Models   : cond_mpd_unet

# All-shapes conditional (single training, shape-conditioned generation):
python flow_matching_runner.py --dataset all_shapes --model cond_mpd_unet
"""
...
def _make_NH_base(nxi):
    ...
    n_pts = np.array([
        [N_lo + 0.25*(N_hi-N_lo), 0.15],
        [N_lo + 0.25*(N_hi-N_lo), 0.85],
        [N_lo + 0.75*(N_hi-N_lo), 0.15],
        [N_lo + 0.75*(N_hi-N_lo), 0.85],
    ])
    bridge = np.array([
        [H_lo + 0.25*(H_hi-H_lo), 0.85],
    ])
    h_pts = np.array([
        [H_lo + 0.25*(H_hi-H_lo), 0.15],
        [H_lo + 0.25*(H_hi-H_lo), 0.50],
        [H_lo + 0.75*(H_hi-H_lo), 0.50],
        [H_lo + 0.75*(H_hi-H_lo), 0.85],
        [H_lo + 0.75*(H_hi-H_lo), 0.15],
    ])
...
def _load_all_shapes(nxi, nd, batch_size, noise_std, device, **kw):
    ...
    ergo_raw = _load_ergodic_raw(1, nxi)
    ergo_proto = ergo_raw[0]
    ergo_base = np.tile(ergo_proto[None], (counts[3], 1, 1))
...
def _draw_arch(ax, model_name, nxi, nd, D, params):
    ...
    rows = [
        ...
        (10.0,'① MPD Tokenize',  f'→(B,{D},{nxi})',          '#FFF8E1'),
        (8.70,'② Time Emb', f't→R^{D}',                  '#F3E5F5'),
        (8.70, '③ Shape Enc', f'ref_cps→R^{D} (+ Time)', '#E8EAF6'),
...
def run_conditional(args):
    ...
    print(f"\n{'='*65}")
    print(f"  Conditional training — {args.model}")
    print(f"{'='*65}")
...
def run_overfit(args):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n{'='*65}")
    print(f"  Extreme Overfitting / Visualization — Ergodic Single Sample")
    print(f"{'='*65}")
    ...
    models_to_train = ['cond_mpd_unet']
    trained_models = {}

    for m_name in models_to_train:
        print(f"\nTraining/Loading {m_name}...")
        model = MODEL_BUILDERS[m_name](nxi=args.nxi, nd=args.nd, D=args.D).to(device)
        params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"  Params: {params:,}")
        
        # Note: if the checkpoints in checkpoints/ folder exist and are given, we use them.
        load_path = f"{args.load_model}" if args.load_model else None
        if load_path and os.path.isfile(load_path):
            print(f"  Loading pretrained {m_name} from {load_path}")
    ...
    fig.suptitle(f'Visualizing Learned Trajectories — Ergodic DB', fontsize=14, fontweight='bold', color='#1A1A2E')
    ...
    for row, m_name in enumerate(models_to_train):
        ...
        _draw_traj(ax_traj, ergo_proto, x1.cpu().numpy(), gen_cps, f"Model: {m_name}", args.bspline_pts, args.bspline_deg)
...
def parse_args():
    ...
    p.add_argument('--model',       default='cond_mpd_unet',  choices=list(MODEL_BUILDERS))
    p.add_argument('--dataset',     default='all_shapes', choices=['all_shapes'])
    p.add_argument('--overfit',     action='store_true',   help='Overfit or visualize single ergodic sample')
    ...
    p.add_argument('--load_model',  type=str,   default=None, help='Path to load pretrained model (.pt)')
    return p.parse_args()
