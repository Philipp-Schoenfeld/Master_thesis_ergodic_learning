r"""
flow_matching_cond_mpd_unet_char_film.py
====================================
Conditional MPD (Motion Planning Diffusion) Flow Matching Network.

FiLM architecture copy: 
Conditioning is injected into every U-Net block via Feature-wise Linear Modulation
(FiLM), dynamically scaling and shifting features rather than simple addition.
"""
...
# ---------------------------------------------------------------------------
# 6. Full Conditional Model (FiLM)
# ---------------------------------------------------------------------------

class CondMpdUNetFlowNetwork(nn.Module):
    """
    Conditional MPD UNet for trajectory generation.
    Conditioned on a reference trajectory via ShapeEncoderMPD and FiLM.
    """
