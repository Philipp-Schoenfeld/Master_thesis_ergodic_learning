#!/usr/bin/env python3
"""
flow_matching_runner_length_loss.py
====================================
Kopie von `flow_matching_runner_length.py`, verdrahtet gegen
`flow_matching_cond_particles_length_loss` statt `..._length`: derselbe
Trainingsablauf, zusaetzlich mit einem expliziten Laengen-Loss-Term
(`--lambda_length`) neben der reinen FiLM-Konditionierung.
...
from flow_matching_cond_particles_length_loss import (
...
def _save_checkpoint(model, optimizer, scheduler, epoch, loss, args):
    ...
    ckpt_dict['lambda_length'] = getattr(args, 'lambda_length', 0.0)
            ...
            for i in range(0, N, args.mini_batch):
                ...
                with torch.autocast(
                    ...
                    loss, parts = loss_fn(
                        ...
                        lambda_length=getattr(args, 'lambda_length', 0.0),
                    )
                loss.backward()
...
def run(args):
    ...
    # Marker fuer den expliziten Laengen-Loss-Term. Ohne ihn waeren die
    # Checkpoints dieses Zweigs (FiLM + Loss) und des reinen FiLM-Zweigs
    # (flow_matching_runner_length.py) namensgleich, sobald derselbe TAG
    # verwendet wird — und der Vergleich, den dieser Lauf liefern soll, waere
    # nicht mehr nachvollziehbar.
    if getattr(args, 'lambda_length', 0.0) > 0.0:
        args.run_str += f"_LENLOSS-w{args.lambda_length:g}"
...
def parse_args():
    ...
    p.add_argument('--lambda_length', type=float, default=0.0,
                   help='Gewicht des expliziten Laengen-Loss-Terms auf der '
                        'Sehnenlaenge der vorhergesagten Kontrollpunkte '
                        '(relativer Fehler gegen die Zielvorgabe). 0 '
                        '(Vorgabe) deaktiviert ihn, dann verhaelt sich dieser '
                        'Zweig identisch zur reinen FiLM-Konditionierung in '
                        'flow_matching_runner_length.py.')
