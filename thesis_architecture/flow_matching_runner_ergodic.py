...
def _build_dataset(train_shapes, holdout_shapes, copies_per_char, S, device):
    """
...
def _save_viz(model, holdout_shapes, holdout_spec, holdout_densities, title, ep_num, args, device, tag=None):
    ...
    viz_path = os.path.join(out_dir, f'viz_ergodic_holdout_{run_str}{tag_str}_ep{ep_num:04d}.png')
    model.eval()
    with torch.no_grad():
...
def _draw_traj(ax, base, gen_cps, density_json, title, bspline_pts=512, bspline_deg=5):
    ...
    if density_json:
        ...
        pdf_grid, _, _ = pdf_on_grid(shape_def, resolution=64)
        if pdf_grid.max() > 0:
    ...
    for idx, lbl in enumerate(labels):
        ...
        if lam is not None:
            ...
            ax.text(0.02, 0.02, f"lam=[{lam_str}]",
                    transform=ax.transAxes, fontsize=5, color='#888', alpha=0.7,
...
def run(args):
    ...
    args.run_str = f"flow_matching_ergodic_{args.timestamp}_S{args.S}_nxi{args.nxi}_D{args.D}_C{args.copies_per_char}_flip{args.p_flip}"
    ...
    x1, holdout_spec = _build_dataset(
