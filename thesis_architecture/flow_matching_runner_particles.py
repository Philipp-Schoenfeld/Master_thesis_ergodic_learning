...
def _load_shapes(nxi, grid_res=128):
    ...
    for blob, label, split, density_params_str in rows:
...
def _build_dataset(train_shapes, train_densities, holdout_shapes, holdout_densities, copies_per_char, n_particles, device, sample_mode='uniform'):
    all_x1 = []
    all_indices = []
    ...
    return x1, shape_indices, density_grids_stack, holdout_particles
...
def train(model, x1_clean, shape_indices, density_grids_stack, loss_fn, args, holdout_shapes=None, holdout_particles=None, holdout_densities=None):
...
def run(args):
    ...
    args.run_str = f"flow_matching_particle_ergodic_{args.timestamp}_nxi{args.nxi}_D{args.D}_N{args.n_particles}_C{args.copies_per_char}_flip{args.p_flip}"
    ...
    train_shapes, holdout_shapes, train_densities, holdout_densities = _load_shapes(args.nxi, grid_res=128)
    print(f"  Loaded {len(train_shapes)} train shapes and {len(holdout_shapes)} val shapes from DB")

    x1, shape_indices, density_grids_stack, holdout_particles = _build_dataset(
    ...
    if args.load_model and os.path.isfile(args.load_model):
        ...
        train(model, x1, shape_indices, density_grids_stack, compute_particle_cfm_loss, args,
              holdout_shapes=holdout_shapes, holdout_particles=holdout_particles, holdout_densities=holdout_densities)
