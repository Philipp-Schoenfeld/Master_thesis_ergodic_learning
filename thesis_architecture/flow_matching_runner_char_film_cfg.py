#!/usr/bin/env python3
"""
flow_matching_runner_char_film_cfg.py — Flow-matching trainer for character/shape trajectories.

Conditions on reference trajectories using FiLM injection AND Classifier-Free Guidance (CFG).

Usage:
  python flow_matching_runner_char_film_cfg.py                                       # train
  python flow_matching_runner_char_film_cfg.py --load_model checkpoints/cond_mpd_char_traj_film_cfg.pt  # viz only
"""
...
# IMPORT FROM NEW CFG MODEL
from flow_matching_cond_mpd_unet_char_film_cfg import (
...
def _build_dataset(shapes, holdout_labels, copies_per_char, device):
    ...
    Stores CLEAN copies only — augmentation happens on-the-fly in the training loop.
    """
    ...
    for ep in pbar:
        ...
        for i in range(0, N, args.mini_batch):
            ...
            with torch.autocast(
                ...
                # Pass p_drop to the loss function for CFG condition dropout
                loss = loss_fn(model, x1[idx], ref_cps[idx], p_drop=args.p_drop)
            loss.backward()
...
def visualise_set(model, shapes_dict, title_prefix, save_path,
    ...
    for idx, lbl in enumerate(labels):
        ...
        gen = generate_cond_trajectories(
            ...
            cfg_weight=args.cfg_weight  # Pass CFG weight
        ).cpu().numpy()
...
def run(args):
    ...
    print(f"\n{'=' * 65}")
    print(f"  Character Flow Matching (FiLM + CFG + PosEnc)")
    print(f"  device={device}  nxi={args.nxi}  D={args.D}  epochs={args.epochs}")
    print(f"  CFG config: p_drop={args.p_drop}  cfg_weight={args.cfg_weight}")
    print(f"{'=' * 65}")
    ...
    visualise_set(
        model, viz_train,
        'Training Shapes — Ground Truth vs Generated (CFG)',
        os.path.join(out_dir, 'char_train_generation_film_cfg.png'),
        args, device,
    )
    ...
    visualise_set(
        ...
        'HELD-OUT Shapes (never seen during training) — Ground Truth vs Generated (CFG)',
        os.path.join(out_dir, 'char_holdout_generation_film_cfg.png'),
        args, device,
    )
...
def parse_args():
    ...
    # CFG Hyperparameters
    p.add_argument('--p_drop',          type=float, default=0.1, help="Probability of dropping the condition during training")
    p.add_argument('--cfg_weight',      type=float, default=2.0, help="Strength of the CFG extrapolation during inference")
