#!/usr/bin/env python3
"""
flow_matching_runner_char_film.py — Flow-matching trainer for character/shape trajectories.

Conditions on reference trajectories using FiLM injection in every U-Net block.

Usage:
  python flow_matching_runner_char_film.py                                       # train
  python flow_matching_runner_char_film.py --load_model checkpoints/cond_mpd_char_traj_film.pt  # viz only
"""
...
# IMPORT FROM NEW FILM MODEL
from flow_matching_cond_mpd_unet_char_film import (
...
def train(model, x1, ref_cps, loss_fn, args):
    ...
    if args.resume and os.path.isfile(args.resume):
        ckpt = torch.load(args.resume, map_location=x1.device, weights_only=True)
...
def run(args):
    ...
    print(f"\n{'=' * 65}")
    print(f"  Character Flow Matching (FiLM trajectory conditioning)")
    ...
    visualise_set(
        model, viz_train,
        'Training Shapes — Ground Truth vs Generated (FiLM)',
        os.path.join(out_dir, 'char_train_generation_film.png'),
        args, device,
    )
    ...
    visualise_set(
        ...
        'HELD-OUT Shapes (never seen during training) — Ground Truth vs Generated (FiLM)',
        os.path.join(out_dir, 'char_holdout_generation_film.png'),
        args, device,
    )
...
def parse_args():
    ...
    p.add_argument('--D',   type=int, default=256) # Defaulted to 256 for bigger dataset
    # training
