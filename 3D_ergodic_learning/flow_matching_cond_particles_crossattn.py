...
class ParticleCrossAttnFlowNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor,
        ...
        dieselbe Stelle wie in der 2D-Fassung. Bleibt es None, ist der Aufruf
        Zeichen fuer Zeichen der alte.
        """
        tokens = self.mpd_layer(x)
        tokens = tokens + self.pos_emb
        time_cond = self.time_emb(t)

        if self.start_cond and start is not None:
            emb = self.start_emb(start)
            if start_drop_mask is not None:
                m = start_drop_mask.view(-1, 1).to(emb.dtype)
                emb = emb * (1.0 - m) + self.null_start_token * m
            time_cond = time_cond + emb
