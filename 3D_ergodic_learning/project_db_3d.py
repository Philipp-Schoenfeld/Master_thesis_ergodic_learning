...
(Kugel, Wuerfel, Platonische Koerper, ...) nicht notwendig nahe an der Bahn
und riss dadurch lange Spruenge hinein statt Luecken. `n_points` je Eintrag
ist deshalb variabel, was die bestehende Kontrollpunkt-Auswahl in
`data_surfaces.py` bereits unterstuetzt. Der Anteil verworfener Punkte
(`miss_frac`/`fehlschuss`) und der groesste verbleibende Sprung (`jump_max`)
werden weiterhin je Eintrag mitgeschrieben und sind die wichtigsten
Guetemasse dieser Datenbank; Eintraege ueber `--max_fehlschuss` (Default
0.35) werden gar nicht erst geschrieben.
...
class Projektor:
    ...
    def strahlen(self, xy):
        """201 Strahlen auf die Bahnpunkte.

        -> (Trefferpunkte, Normalen, ok). Trefferpunkte/Normalen enthalten nur
        die Eintraege mit `ok=True`, also kuerzer als `xy` bei Fehlschuessen;
        `ok` bleibt in voller Laenge fuer `fehlschuss`/`treffer_ok`.

        Fehlschuesse wurden frueher durch den *naechstgelegenen* Punkt auf der
        gesamten Flaeche ersetzt. Der liegt zwar auf der Flaeche, aber bei
        konvexen Formen (Kugel, Wuerfel, Platonische Koerper, ...) nicht
        notwendig nahe an der Bahn — je nach Kruemmung kann er auf einer ganz
        anderen Stelle landen und riss dadurch lange Spruenge in die Bahn statt
        Luecken (siehe Chat vom 2026-09-04: Median-Sprung bei der Kugel lag bei
        0.29-0.39 trotz nur ~3% Fehlschuss). Der Rohpunkt wird jetzt stattdessen
        verworfen; `n_points` ist dadurch je Eintrag variabel, was die
        bestehende Kontrollpunkt-Auswahl in `data_surfaces.py`
        (`np.linspace(0, n_points-1, nxi)`) bereits unterstuetzt.
        """
...
# ── Ein Eintrag ──────────────────────────────────────────────────────────────
def projiziere(proj, d2, xy, standoff=0.0, n_particles=512, seed=0):
    """-> dict mit Bahn (T,3), Rahmen (T,6), Partikeln (N,4) und Guetemassen.

    T kann kleiner als `len(xy)` sein, siehe `Projektor.strahlen`. Bleiben
    weniger als zwei Punkte uebrig (nur bei extremem Fehlschussanteil), wird
    `fehlschuss=1.0` erzwungen, damit der Aufrufer den Eintrag ueber
    `--max_fehlschuss` sicher verwirft, statt an der leeren Tangenten-
    Berechnung in `frames_from_normals` zu scheitern.
    """
    hitp, hitn, ok = proj.strahlen(xy)
    if len(hitp) < 2:
        return dict(pos=np.zeros((0, 3), np.float32),
                    rot6=np.zeros((0, 6), np.float32),
                    parts=np.zeros((n_particles, 4), np.float32),
                    flaeche=proj.pts.astype(np.float32),
                    gewicht=np.zeros(len(proj.pts), np.float32),
                    treffer_ok=ok, fehlschuss=1.0,
                    standoff=0.0, standoff_sd=0.0, sprung_max=0.0,
                    sprung_mittel=0.0, senk_max=0.0, getroffen=0.0)
