...
def B_onp(x, k, i, knots, max_k, n_derivatives=0):
    c = onp.zeros(len(knots) - k - 1)
    c[i] = 1.0
    bspl = scipy.interpolate.BSpline(knots, c, k)
    if n_derivatives > 0:
        bspl = bspl.derivative(nu=n_derivatives)
    return bspl(x)
...
if __name__ == '__main__':
    ...
    print('Starting JAX sampling (this may take a while to compile/run)...')
